...
export const adminStats = createServerFn({ method: "POST" })
  .inputValidator((i: { password: string }) => z.object({ password: z.string().min(1).max(200) }).parse(i))
  .handler(async ({ data }) => {
    ...
    const [
      ...
      db
        .from("payments")
        .select("id, amount, currency, status, test_mode, created_at")
        .order("created_at", { ascending: false })
        .limit(10),
      db
        .from("game_sessions")
        .select("id, score, status, attempt_number, started_at")
        .order("started_at", { ascending: false })
        .limit(10),
      db
        .from("challenges")
        .select("challenge_code, challenger_score, opens, created_at")
        .order("created_at", { ascending: false })
        .limit(10),
      db
        .from("scores")
